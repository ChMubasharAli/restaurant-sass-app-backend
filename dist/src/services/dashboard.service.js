...
export class DashboardService {
    ...
    async getDashboardStats() {
        try {
            const [orderStats, reservationStats, restaurantSettings] = await Promise.all([
            ...
            const todayOrders = await this.orderRepository.prisma.order.count({
                where: {
                    createdAt: {
                        gte: todayStart,
                        lte: todayEnd,
            ...
            // Get today's revenue
            const todayRevenue = await this.orderRepository.prisma.order.aggregate({
                _sum: {
                    totalAmount: true,
                },
                where: {
            ...
            const recentOrders = await this.orderRepository.prisma.order.findMany({
                take: 10,
                orderBy: {
                    createdAt: "desc",
                },
            ...
            // Get orders by type
            const ordersByType = await this.orderRepository.prisma.order.groupBy({
                by: ["orderType"],
                _count: true,
            });
            // Get payment method distribution
            const paymentsByMethod = await this.orderRepository.prisma.order.groupBy({
                by: ["paymentMethod"],
                _count: true,
                _sum: {
                    totalAmount: true,
            ...
            const recentReservations = await this.reservationRepository.prisma.reservation.findMany({
                take: 10,
                orderBy: {
                    createdAt: "desc",
                },
            ...
            const todayReservations = await this.reservationRepository.prisma.reservation.count({
                where: {
                    reservationDate: {
                        gte: todayStart,
                        lte: todayEnd,
    ...
    async getSalesReport(startDate, endDate) {
        try {
            ...
            const salesData = await this.orderRepository.prisma.order.findMany({
                where,
                select: {
                    id: true,
                    orderNumber: true,
    ...
    async getOrderAnalytics(days = 30) {
        try {
            ...
            const orders = await this.orderRepository.prisma.order.findMany({
                where: {
                    createdAt: {
                        gte: startDate,
                    },
