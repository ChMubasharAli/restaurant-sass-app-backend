...
export class OrderService {
    ...
    async createOrder(data) {
        ...
        for (const item of data.items) {
            ...
            if (!menuItem.isAvailable) {
                throw new ValidationError(`Menu item "${menuItem.name}" is currently sold out`);
            }
    ...
    async updateOrderStatus(id, status) {
        ...
        if (!validStatuses.includes(status)) {
            throw new ValidationError("Invalid order status. Must be one of: PENDING, PREPARING, READY, COMPLETED, CANCELLED");
        }
        // Update order status
