...
export class ReservationService {
    ...
    async createReservation(data) {
        ...
        const isAvailable = await this.reservationRepository.checkAvailability(new Date(data.reservationDate), data.timeSlot);
        if (!isAvailable) {
            throw new ConflictError("This time slot is already reserved. Please choose another time.");
        }
    ...
    async cancelReservation(id) {
        ...
        const reservation_date = await this.reservationRepository.updateStatus(id, "CANCELLED", "Cancelled - slot now available");
    ...
    async getReservationSchedule(date) {
        return this.reservationRepository.getReservationSchedule(date ? new Date(date) : undefined);
    }
    async getReservationStats() {
        return this.reservationRepository.getReservationStats();
    }
    async checkSlotAvailability(date, timeSlot) {
        const isAvailable = await this.reservationRepository.checkAvailability(new Date(date), timeSlot);
        return {
            date,
            timeSlot,
