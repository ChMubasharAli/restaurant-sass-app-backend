...
export class AdminService {
    ...
    async changePassword(id, currentPassword, newPassword, confirmPassword) {
        ...
        const isCurrentPasswordValid = this.verifyPassword(currentPassword, fullAdmin.password);
        if (!isCurrentPasswordValid) {
            throw new ValidationError("Current password is incorrect");
        }
        // Hash new password and update
        const hashedNewPassword = this.hashPassword(newPassword);
        const updatedAdmin = await this.adminRepository.updatePassword(id, hashedNewPassword);
        return {
            ...updatedAdmin,
